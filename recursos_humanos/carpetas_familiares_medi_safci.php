...
$(function () {
 ...
 <?php
$numero = 0;
$sql = " SELECT fecha_registro FROM carpeta_familiar WHERE estado='CONSOLIDADO' GROUP BY fecha_registro ORDER BY fecha_registro";
...
<?php
$sql7 = " SELECT idcarpeta_familiar, fecha_registro FROM carpeta_familiar WHERE estado='CONSOLIDADO' AND fecha_registro='$row[0]'";
...
<table width="1000" border="1" align="center" cellspacing="0">
    ...
    $sql.=" AND area_influencia.idtipo_area_influencia=tipo_area_influencia.idtipo_area_influencia AND carpeta_familiar.idarea_influencia=area_influencia.idarea_influencia ORDER BY carpeta_familiar.idcarpeta_familiar DESC LIMIT 1000 ";
    $result = mysqli_query($link,$sql);
    if ($row = mysqli_fetch_array($result)){
    mysqli_field_seek($result,0);           
    while ($field = mysqli_fetch_field($result)){
