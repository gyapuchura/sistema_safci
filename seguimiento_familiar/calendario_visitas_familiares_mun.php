...
$sql.=" AND seguimiento_cf.idintegrante_cf=integrante_cf.idintegrante_cf AND carpeta_familiar.idmunicipio='$idmunicipio_ss' ORDER BY visita_cf.fecha_visita ";
...
?>
			...
			title: '<?php echo $row[1]." ".$row[2]." ".$row[3];?> - <?php echo $row[4];?> ',
					...
					url: 'valida_visita_calendario_mun.php?idcarpeta_familiar=<?php echo $row[0];?>',
					className: '<?php
 if ($row[6]=='1') {
   echo "info";
 }
		    ...
		    <tr>
		      ...
		      <td width="618" style="color: #365DCE"><h2 class="text-info" style="text-align: center">CALENDARIO VISITAS FAMILIARES - SAFCI</h2>
                ...
                <?php
					...
					$sql_mun.=" WHERE idmunicipio='$idmunicipio_ss' ";
					$result_mun = mysqli_query($link,$sql_mun);
					$row_mun = mysqli_fetch_array($result_mun);?>
                </span>
              <h2 style="text-align: center"><?php echo "MUNICIPIO : ".$row_mun[1];?></h2></td>
		      <td width="124">&nbsp;</td>
	        </tr>
	      </tbody>
</table>
