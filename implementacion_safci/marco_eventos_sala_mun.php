...
<html>
	<head>
		...
		<title>EVENTOS DE NOTIFICACION INMEDIATA - MUNICIPIO</title>
...
$(function () {
    $('#container').highcharts({
        ...
        title: {
            text: 'EVENTO: <?php echo mb_strtoupper($row_ev[1]);?> - MUNICIPIO : <?php echo $row_mun[1];?>'
        },
        legend: {
            layout: 'vertical',
            align: 'left',
 ...
 <?php
$numero = 0;
$sql = " SELECT semana_ep FROM notificacion_ep WHERE gestion ='$gestion' AND estado='CONSOLIDADO' AND idmunicipio='$idmunicipio' GROUP BY semana_ep ORDER BY semana_ep";
...
$numero = 0;
$sql = " SELECT semana_ep FROM notificacion_ep WHERE gestion ='$gestion' AND idmunicipio='$idmunicipio' AND estado='CONSOLIDADO' GROUP BY semana_ep ORDER BY semana_ep ";
$result = mysqli_query($link,$sql);
...
$sql7.= " AND notificacion_ep.gestion='$gestion' AND notificacion_ep.semana_ep='$row[0]' AND notificacion_ep.idmunicipio='$idmunicipio' ";
...
$numeroa = 0;
$sqla = " SELECT semana_ep FROM notificacion_ep WHERE gestion ='$gestion' AND idmunicipio='$idmunicipio' AND estado='CONSOLIDADO' GROUP BY semana_ep ORDER BY semana_ep ";
...
$sqlca.= " AND notificacion_ep.gestion='$gestion' AND notificacion_ep.semana_ep='$rowa[0]' AND notificacion_ep.idmunicipio='$idmunicipio' ";
...
<div id="container" style="min-width: 300px; height: 350px; margin: 0 auto"></div>
    ...
    $sql.=" AND notificacion_ep.estado='CONSOLIDADO' AND registro_evento_notificacion.idevento_notificacion='$idevento_notificacion' AND notificacion_ep.idmunicipio='$idmunicipio' ORDER BY notificacion_ep.semana_ep ";
    $result = mysqli_query($link,$sql);
    if ($row = mysqli_fetch_array($result)){
    mysqli_field_seek($result,0);           
    while ($field = mysqli_fetch_field($result)){
