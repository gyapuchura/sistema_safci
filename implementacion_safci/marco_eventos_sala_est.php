...
<html>
	<head>
		...
		<title>EVENTOS DE NOTIFICACION INMEDIATA - ESTABLECIMIENTO</title>
...
$(function () {
    $('#container').highcharts({
        ...
        title: {
            text: 'EVENTO: <?php echo mb_strtoupper($row_ev[1]);?> - ESTABLECIMIENTO <?php echo $row_est[1];?>'
        },
        legend: {
            layout: 'vertical',
            align: 'left',
 ...
 <?php
$numero = 0;
$sql = " SELECT semana_ep FROM notificacion_ep WHERE gestion ='$gestion' AND estado='CONSOLIDADO' AND idestablecimiento_salud='$idestablecimiento_salud' GROUP BY semana_ep ORDER BY semana_ep";
...
$numero = 0;
$sql = " SELECT semana_ep FROM notificacion_ep WHERE gestion ='$gestion' AND idestablecimiento_salud='$idestablecimiento_salud' AND estado='CONSOLIDADO' GROUP BY semana_ep ORDER BY semana_ep ";
$result = mysqli_query($link,$sql);
...
$sql7.= " AND notificacion_ep.gestion='$gestion' AND notificacion_ep.semana_ep='$row[0]' AND notificacion_ep.idestablecimiento_salud='$idestablecimiento_salud' ";
...
$numeroa = 0;
$sqla = " SELECT semana_ep FROM notificacion_ep WHERE gestion ='$gestion' AND idestablecimiento_salud='$idestablecimiento_salud' AND estado='CONSOLIDADO' GROUP BY semana_ep ORDER BY semana_ep ";
...
$sqlca.= " AND notificacion_ep.gestion='$gestion' AND notificacion_ep.semana_ep='$rowa[0]' AND notificacion_ep.idestablecimiento_salud='$idestablecimiento_salud' ";
...
<div id="container" style="min-width: 300px; height: 350px; margin: 0 auto"></div>
    ...
    $sql.=" AND notificacion_ep.estado='CONSOLIDADO' AND registro_evento_notificacion.idevento_notificacion='$idevento_notificacion' AND notificacion_ep.idestablecimiento_salud='$idestablecimiento_salud' ORDER BY notificacion_ep.semana_ep ";
    $result = mysqli_query($link,$sql);
    if ($row = mysqli_fetch_array($result)){
    mysqli_field_seek($result,0);           
    while ($field = mysqli_fetch_field($result)){
