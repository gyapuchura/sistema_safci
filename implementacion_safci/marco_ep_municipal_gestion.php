...
$(function () {
    $('#container').highcharts({
        ...
        title: {
            text: 'VIGILANCIA: <?php echo mb_strtoupper($row_sos[1]);?> - MUNICIPIO: <?php echo mb_strtoupper($row_m[1]);?> - GESTIÓN <?php echo $gestion;?>'
        },
        legend: {
            layout: 'vertical',
            align: 'left',
...
<div id="container" style="min-width: 300px; height: 350px; margin: 0 auto"></div>
    ...
    $sql.=" WHERE registro_enfermedad.idnotificacion_ep=notificacion_ep.idnotificacion_ep AND notificacion_ep.idestablecimiento_salud=establecimiento_salud.idestablecimiento_salud AND notificacion_ep.estado='CONSOLIDADO' AND registro_enfermedad.cifra !='0' ";
    ...
    ?>
		    <tr>
              ...
              <a href="marco_ep_establecimiento_gestion.php?idsospecha_diag_estab=<?php echo $idsospecha_diag_mun;?>&idestablecimiento_salud=<?php echo $row[0];?>&gestion=<?php echo $gestion?>" target="_blank" class="Estilo12" style="font-size: 12px; font-family: Arial;" onClick="window.open(this.href, this.target, 'width=1000,height=400,scrollbars=YES,top=60,left=400'); return false;">REPORTE</a>
              </td>
		      <td style="font-size: 12px; color: #2D56CF; font-family: Arial; text-align: center;">
              <a href="piramide_sospechas_estab_gestion.php?idsospecha_diag_estab=<?php echo $idsospecha_diag_mun;?>&idestablecimiento_salud=<?php echo $row[0];?>&gestion=<?php echo $gestion?>" target="_blank" class="Estilo12" onClick="window.open(this.href, this.target, 'width=1000,height=400,scrollbars=YES,top=50,left=300'); return false;">GRUPOS</a></td>
		      <td style="font-size: 12px; color: #2D56CF; font-family: Arial; text-align: center;">
              <a href="detalle_notificaciones_estab_gestion.php?idestablecimiento_salud=<?php echo $row[0];?>&idmunicipio=<?php echo $idmunicipio;?>&idsospecha_diag=<?php echo $idsospecha_diag_mun?>&gestion=<?php echo $gestion?>" target="_blank" class="Estilo12" onClick="window.open(this.href, this.target, 'width=800,height=500,scrollbars=YES,top=50,left=300'); return false;">NOTIFICACIONES</a></td>
            </td>
	        </tr>
            <?php
        $numero=$numero+1;
