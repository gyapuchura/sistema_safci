...
<html>
	<head>
		...
		<title>ESTABLECIMIENTOS_SUBSECTOR</title>
...
$(function () {
    $('#container').highcharts({
        ...
        title: {
            text: 'ESTABLECIMIENTOS DE SALUD POR SUB-SECTOR'
        },
        tooltip: {
...
<?php
$sql0 = " SELECT idestablecimiento_salud, idsubsector_salud FROM establecimiento_salud ";
$result0 = mysqli_query($link,$sql0);
$total = mysqli_num_rows($result0);

$numero = 0;
$sql = " SELECT idsubsector_salud FROM establecimiento_salud  GROUP BY idsubsector_salud ORDER BY idsubsector_salud ";
...
$sql_c= " SELECT idestablecimiento_salud, idsubsector_salud FROM establecimiento_salud WHERE idsubsector_salud='$row[0]'  ";
...
$sql_ta = " SELECT idestablecimiento_salud, idsubsector_salud FROM establecimiento_salud ";
$result_ta = mysqli_query($link,$sql_ta);
$total_ta = mysqli_num_rows($result_ta);

$numeroa = 1;
$sqla = " SELECT idsubsector_salud FROM establecimiento_salud GROUP BY idsubsector_salud ORDER BY idsubsector_salud ";
...
$sql_ta = " SELECT idsubsector_salud, subsector_salud FROM subsector_salud WHERE idsubsector_salud='$rowa[0]' ";
$result_ta = mysqli_query($link,$sql_ta);
$row_ta = mysqli_fetch_array($result_ta);

$sql_ca = " SELECT idestablecimiento_salud, idsubsector_salud FROM establecimiento_salud WHERE idsubsector_salud='$rowa[0]' ";
