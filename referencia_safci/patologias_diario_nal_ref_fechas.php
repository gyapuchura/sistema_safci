...
<html>
	<head>
		...
		<title>MEDI-APS DIAGNOSTICOS REFERENCIA - DIARIAS</title>
 ...
 <?php
$numero = 0;
$sql = " SELECT referencia_hc.fecha_registro FROM referencia_hc, diagnostico_presuntivo WHERE diagnostico_presuntivo.idreferencia_hc=referencia_hc.idreferencia_hc ";
$sql.= " AND diagnostico_presuntivo.idpatologia='$idpatologia' AND diagnostico_presuntivo.fecha_registro BETWEEN '$inicio' AND '$finalizacion' GROUP BY referencia_hc.fecha_registro ORDER BY referencia_hc.fecha_registro ";
...
?>
        ...
        yAxis: {
            title: {
                text: 'DIAGNOSTICOS PRESUNTIVOS PARA REFERENCIA DIARIOS'
            }
        },
        tooltip: {
            shared: true,
        ...
        series: [{
            name: 'DIAGNÓSTICO PRESUNTIVO PARA REFERENCIA',
            data: [
...
<?php
$sql7 = " SELECT diagnostico_presuntivo.iddiagnostico_presuntivo  FROM diagnostico_presuntivo, referencia_hc ";
$sql7.= " WHERE diagnostico_presuntivo.idreferencia_hc=referencia_hc.idreferencia_hc AND diagnostico_presuntivo.fecha_registro='$row[0]'  ";
$sql7.= " AND diagnostico_presuntivo.idpatologia='$idpatologia' AND diagnostico_presuntivo.fecha_registro BETWEEN '$inicio' AND '$finalizacion' ";
...
<table width="1000" border="1" align="center" cellspacing="0">
		  <tbody>
		    <tr>
              ...
              <td width="200" style="font-size: 12px; color: #2D56CF; font-family: Arial; text-align: center;">ESTADO REFERENCIA</td>
              <td width="200" style="font-size: 12px; color: #2D56CF; font-family: Arial; text-align: center;">ESPECIALIDAD MÉDICA</td>
    ...
    $sql.=" especialidad_medica.especialidad_medica,referencia_hc.fecha_registro, referencia_hc.hora_registro, referencia_hc.idusuario, nombre.fecha_nac, genero.genero  ";
    $sql.=" FROM referencia_hc, nombre, estado_referencia, especialidad_medica, departamento, municipios, establecimiento_salud, diagnostico_presuntivo, genero ";
    $sql.=" WHERE referencia_hc.idnombre=nombre.idnombre AND diagnostico_presuntivo.idreferencia_hc=referencia_hc.idreferencia_hc AND nombre.idgenero=genero.idgenero ";
    ...
    $sql.=" AND referencia_hc.idespecialidad_medica=especialidad_medica.idespecialidad_medica AND diagnostico_presuntivo.idpatologia='$idpatologia' AND diagnostico_presuntivo.fecha_registro BETWEEN '$inicio' AND '$finalizacion' ORDER BY referencia_hc.idreferencia_hc DESC ";
    $result = mysqli_query($link,$sql);
    if ($row = mysqli_fetch_array($result)){
    mysqli_field_seek($result,0);           
    while ($field = mysqli_fetch_field($result)){
    ...
    ?>
		    <tr>
              ...
              <a href="imprime_formulario_d7.php?idreferencia_hc=<?php echo $row[0];?>" target="_blank" onClick="window.open(this.href, this.target, 'width=1000,height=900,top=50, left=200, scrollbars=YES'); return false;">
              <?php echo $row[1];?></a>  </td>
