...
<html>
    ...
    $('#container').highcharts({
        ...
        title: {
            text: 'REPORTE DIAGNÓSTICOS POR TELESALUD - NACIONAL'
        },
        subtitle: {
...
$numero = 0;
$sql = " SELECT p.idpatologia, p.patologia, p.cie, COUNT(d.iddiagnostico_teleconsulta) as total ";
$sql.= " FROM diagnostico_teleconsulta d, patologia p, atencion_psafci a ";
$sql.= " WHERE d.idpatologia=p.idpatologia AND d.idatencion_psafci=a.idatencion_psafci ";
...
?>
        ...
        yAxis: {
            min: 0,
            title: { text: ' DIAGNÓSTICOS TELESALUD ', align: 'high' },
...
<h4 align="center" style="font-family: Arial;">Nº DIAGNÓSTICOS POR TELESALUD: 
<?php
$sql_dgt = " SELECT count(d.iddiagnostico_teleconsulta) FROM diagnostico_teleconsulta d, patologia p ";
...
<table width="1000" border="1" align="center" bordercolor="#009999">
    <tr>
        ...
        <td width="400" bgcolor="#FFFFFF" style="font-family: Arial; font-size: 12px;"><span class="Estilo8">DIAGNÓSTICOS</span></td>
        <td width="50" bgcolor="#FFFFFF" align="center" style="font-family: Arial; font-size: 12px;"><span class="Estilo8">CIE</span></td>
            ...
            <a href="reporte_telesalud_depto_fechas.php?iddepartamento=<?php echo $row_d[0];?>&inicio=<?php echo $inicio;?>&finalizacion=<?php echo $finalizacion;?>" target="_blank" class="Estilo12" onClick="window.open(this.href, this.target, 'width=1420,height=820,scrollbars=YES,top=50,left=200'); return false;"><?php echo $row_d[1] ?></a>
            </td>
            <?php } ?>
    ...
    while ($row = mysqli_fetch_array($result)) { ?>
        <tr>
            ...
            <td bgcolor="#FFFFFF" align="center" style="font-family: Arial; font-size: 12px;">
                 <a href="patologias_diario_nal_tele_fechas.php?idpatologia=<?php echo $row[0];?>&inicio=<?php echo $inicio;?>&finalizacion=<?php echo $finalizacion;?>" target="_blank" class="Estilo12" onClick="window.open(this.href, this.target, 'width=1420,height=820,scrollbars=YES,top=50,left=200'); return false;"><?php echo $row[2];?></a>
            </td>
            <?php
            mysqli_data_seek($result_d, 0); // Reiniciamos puntero de departamentos
            while ($row_d = mysqli_fetch_array($result_d)) {
                $sql_dg = " SELECT count(d.iddiagnostico_teleconsulta) FROM diagnostico_teleconsulta d, atencion_psafci a ";
