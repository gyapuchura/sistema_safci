...
$idexamen_complementario = $_GET['idexamen_complementario'];

$sql_ec =" SELECT idexamen_complementario, examen_complementario FROM examen_complementario WHERE idexamen_complementario='$idexamen_complementario' ";
$result_ec = mysqli_query($link,$sql_ec);
$row_ec = mysqli_fetch_array($result_ec);
...
<html>
	<head>
		...
		<title>MEDI-SAFCI ATENCIONES TELESALUD - EXAMEN COMPLEMENTARIO</title>
        ...
        title: {
            text: 'EXÁMEN COMPLEMENTARIO : <?php echo mb_strtoupper($row_ec[1]);?>'
        },
        subtitle: {
 ...
 <?php
$numero = 0;
$sql = " SELECT fecha_registro FROM examen_teleconsulta WHERE idexamen_complementario='$idexamen_complementario' AND fecha_registro BETWEEN '$inicio' AND '$finalizacion' GROUP BY fecha_registro ORDER BY fecha_registro  ";
...
?>
        ...
        yAxis: {
            title: {
                text: 'TELEMETRÍAS DIARIAS'
            }
        },
        tooltip: {
            shared: true,
            valueSuffix: ' Mediciones'
        },
        credits: {
            enabled: false
        },
        ...
        series: [{
            name: '<?php echo mb_strtoupper($row_ec[1]);?>',
            data: [
...
<?php
$sql7 = " SELECT idexamen_teleconsulta FROM examen_teleconsulta WHERE idexamen_complementario='$idexamen_complementario' AND fecha_registro = '$row[0]' ";
...
<div align="center">
<!-- <form action="reporte_diagnosticos_preventivos_excel_ec.php" method="post">
...
<table width="1000" border="1" align="center" cellspacing="0">
    ...
    $sql.=" FROM atencion_psafci, nombre, tipo_consulta, tipo_atencion, departamento, municipios, establecimiento_salud, examen_teleconsulta ";
    $sql.=" WHERE atencion_psafci.idnombre=nombre.idnombre AND examen_teleconsulta.idatencion_psafci=atencion_psafci.idatencion_psafci ";
    ...
    $sql.=" AND atencion_psafci.idtipo_atencion=tipo_atencion.idtipo_atencion AND examen_teleconsulta.idexamen_complementario='$idexamen_complementario'  ";
    $sql.=" AND examen_teleconsulta.fecha_registro BETWEEN '$inicio' AND '$finalizacion' ORDER BY atencion_psafci.idatencion_psafci DESC ";
    $result = mysqli_query($link,$sql);
    if ($row = mysqli_fetch_array($result)){
    mysqli_field_seek($result,0);           
    while ($field = mysqli_fetch_field($result)){
