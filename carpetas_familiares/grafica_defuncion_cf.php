...
$(function () {
    $('#suministro_agua').highcharts({
        ...
        series: [{
            ...
            data: [
                <?php
                    $sql0 = " SELECT count(idintegrante_defuncion) FROM integrante_defuncion ";
                    $result0 = mysqli_query($link,$sql0);
                    $row0 = mysqli_fetch_array($result0);
                    $total = $row0[0];

                    $numero = 0;
                    $sql = " SELECT defuncion_cf FROM integrante_defuncion GROUP BY defuncion_cf ";
                    ...
                    $sql_c= " SELECT count(idintegrante_defuncion) FROM integrante_defuncion WHERE defuncion_cf ='$row[0]' ";
