    ...
    case 1:
        ...
        $sumatoria = $rowa[0];
            ...
            if ($sumatoria <= 7 ) {
                ...
                echo "<h6 style='color: #000000; font-family: arial; font-size: 12px;'>".$row5[0]."</h6>";
            } else {
                if ($sumatoria <= 11 ) {
                    $sql6 = " SELECT riesgo_cf FROM riesgo_cf WHERE idriesgo_cf ='2' ";
                    $result6 = mysqli_query($link,$sql6);
                    $row6 = mysqli_fetch_array($result6);
                    echo "<h6 style='color: #00913f; font-family: arial; font-size: 12px;'>".$row6[0]."</h6>";
                } else {
                    if ($sumatoria <= 17) {
                            $sql7 = " SELECT riesgo_cf FROM riesgo_cf WHERE idriesgo_cf ='3' ";
                            $result7 = mysqli_query($link,$sql7);
                            $row7 = mysqli_fetch_array($result7);
                            echo "<h6 style='color: #0000ff; font-family: arial; font-size: 12px;'>".$row7[0]."</h6>";
                    } else {
                        if ($sumatoria <= 24) {
                                $sql8 = " SELECT riesgo_cf FROM riesgo_cf WHERE idriesgo_cf ='4' ";
                                $result8 = mysqli_query($link,$sql8);
                                $row8 = mysqli_fetch_array($result8);
                                echo "<h6 style='color: #FFA500; font-family: arial; font-size: 12px;'>".$row8[0]."</h6>";
                        } else { 
                            if ($sumatoria <= 35) {
                                    $sql9 = " SELECT riesgo_cf FROM riesgo_cf WHERE idriesgo_cf ='5' ";
                                    $result9 = mysqli_query($link,$sql9);
                                    $row9 = mysqli_fetch_array($result9);
                                    echo "<h6 style='color: #FF0000; font-family: arial; font-size: 12px;'>".$row9[0]."</h6>";
                            } else {  } } } } }    
        break;
    case 2:
