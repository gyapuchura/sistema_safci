...
<html lang="es">
	<head>
		...
		<title>SALUD ALIMENTARIA CF</title>
...
?>
        ...
        series: [
            ...
            <?php 
$numero2 = 0;
$sql2 = " SELECT valor_cf FROM determinante_salud_cf WHERE iddeterminante_salud='4' AND idcat_determinante_salud='19' GROUP BY valor_cf ";
...
<?php
$sql_a =" SELECT count(valor_cf) FROM determinante_salud_cf WHERE iddeterminante_salud='4' AND idcat_determinante_salud='19' AND iditem_determinante_salud='$row3[0]' AND valor_cf='$row2[0]' ";
...
<table width="646" border="1" align="center" bordercolor="#009999">
    <tr>
        ...
        <td width="315" bgcolor="#FFFFFF" style="font-family: Arial; font-size: 12px;"><span class="Estilo8 Estilo1 Estilo2">Durante los últimos tres meses, por falta de dinero u otros factores hubo algún momento en el que:</span></td>
...
<?php
                    $numero = 1;
                    $sql = " SELECT iditem_determinante_salud FROM determinante_salud_cf WHERE iddeterminante_salud='4' AND idcat_determinante_salud='19' GROUP BY iditem_determinante_salud ";
                    $result = mysqli_query($link,$sql);
                    if ($row = mysqli_fetch_array($result)){
                    mysqli_field_seek($result,0);
                    while ($field = mysqli_fetch_field($result)){
                    ...
                    $sql_si= " SELECT count(valor_cf) FROM determinante_salud_cf WHERE iddeterminante_salud='4' AND idcat_determinante_salud='19' AND iditem_determinante_salud='$row[0]' AND valor_cf='1'  ";
                    $result_si = mysqli_query($link,$sql_si);
                    $row_si = mysqli_fetch_array($result_si);
                    $res_si = $row_si[0];

                    $sql_no= " SELECT count(valor_cf) FROM determinante_salud_cf WHERE iddeterminante_salud='4' AND idcat_determinante_salud='19' AND iditem_determinante_salud='$row[0]' AND valor_cf='0'  ";
