...
$(function () {
    $('#corresponde_salud').highcharts({
        ...
        series: [{
            ...
            data: [
                <?php
                    $sql0 = " SELECT count(idintegrante_cf) FROM integrante_cf  ";
                    $result0 = mysqli_query($link,$sql0);
                    $row0 = mysqli_fetch_array($result0);
                    $total = $row0[0];

                    $numero = 0;
                    $sql = " SELECT idnacion FROM integrante_cf GROUP BY idnacion ";
                    ...
                    $sql_t = " SELECT idnacion, nacion FROM nacion WHERE idnacion='$row[0]' ";
                    $result_t = mysqli_query($link,$sql_t);
                    $row_t = mysqli_fetch_array($result_t);

                    $sql_c= " SELECT count(idintegrante_cf) FROM integrante_cf WHERE idnacion='$row[0]' ";
