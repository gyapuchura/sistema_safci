...
<head>
    ...
    <title>REPORTE CARPETAS FAMILIARES POR ESTABLECIMIENTO</title>
</head>
<body>
...
$(function () {
    $('#container').highcharts({
        ...
        title: {
            text: 'REGISTRO DE CARPETAS FAMILIARES - ESTABLECIMIENTO <?php echo mb_strtoupper($establecimiento_salud_cf);?>'
        },
        legend: {
            layout: 'vertical',
            align: 'left',
 ...
 <?php
$numero = 0;
$sql = " SELECT fecha_registro FROM carpeta_familiar WHERE estado='CONSOLIDADO' AND idestablecimiento_salud='$idestablecimiento_salud' GROUP BY fecha_registro ORDER BY fecha_registro";
...
<?php
$sql7 = " SELECT idcarpeta_familiar, fecha_registro FROM carpeta_familiar WHERE estado='CONSOLIDADO' AND idestablecimiento_salud='$idestablecimiento_salud' AND fecha_registro='$row[0]'";
...
<h4 style="font-family: Arial; font-size: 16px; color: #2D56CF; text-align: center;">CARPETAS FAMILIARES A NIVEL DE ESTABLECIMIENTO DE SALUD</h4>
...
<table width="1000" border="1" align="center" cellspacing="0">
    ...
    $sql.=" AND area_influencia.idtipo_area_influencia=tipo_area_influencia.idtipo_area_influencia AND carpeta_familiar.idarea_influencia=area_influencia.idarea_influencia AND carpeta_familiar.idestablecimiento_salud='$idestablecimiento_salud' ORDER BY carpeta_familiar.idcarpeta_familiar DESC ";
    $result = mysqli_query($link,$sql);
    if ($row = mysqli_fetch_array($result)){
    mysqli_field_seek($result,0);           
    while ($field = mysqli_fetch_field($result)){
