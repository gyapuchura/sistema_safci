...
$(function () {
    $('#consumo_diario_alimentos').highcharts({
        ...
        title: {
            text: 'b) Consumo diario de alimentos'
        },
        subtitle: {
...
?>
        ...
        series: [
            ...
            <?php 
$numero2 = 0;
$sql2 = " SELECT valor_cf FROM determinante_salud_cf WHERE iddeterminante_salud='4' AND idcat_determinante_salud='21' GROUP BY valor_cf ";
...
<?php
$sql_a =" SELECT count(valor_cf) FROM determinante_salud_cf WHERE iddeterminante_salud='4' AND idcat_determinante_salud='21' AND iditem_determinante_salud='$row3[0]' AND valor_cf='$row2[0]' ";
...
<?php
                    $numero = 1;
                    $sql = " SELECT iditem_determinante_salud FROM determinante_salud_cf WHERE iddeterminante_salud='4' AND idcat_determinante_salud='21' GROUP BY iditem_determinante_salud ";
                    $result = mysqli_query($link,$sql);
                    if ($row = mysqli_fetch_array($result)){
                    mysqli_field_seek($result,0);
                    while ($field = mysqli_fetch_field($result)){
                    ...
                    $sql_si= " SELECT count(valor_cf) FROM determinante_salud_cf WHERE iddeterminante_salud='4' AND idcat_determinante_salud='21' AND iditem_determinante_salud='$row[0]' AND valor_cf='1'  ";
                    $result_si = mysqli_query($link,$sql_si);
                    $row_si = mysqli_fetch_array($result_si);
                    $res_si = $row_si[0];

                    $sql_no= " SELECT count(valor_cf) FROM determinante_salud_cf WHERE iddeterminante_salud='4' AND idcat_determinante_salud='21' AND iditem_determinante_salud='$row[0]' AND valor_cf='0'  ";
