...
<html>
	<head>
		...
		<title>NIVEL DE INSTRUCCION</title>
...
$(function () {
    $('#corresponde_salud').highcharts({
        ...
        title: {
            text: 'NIVEL DE INSTRUCCIÓN DE LOS INTEGRANTES DE LA FAMILIA - NIVEL NACIONAL'
        },
        subtitle: {
        ...
        series: [{
            ...
            data: [
                <?php
                    ...
                    $numero = 0;
                    $sql = " SELECT idnivel_instruccion FROM integrante_datos_cf GROUP BY idnivel_instruccion ";
                    ...
                    $sql_c= " SELECT count(idintegrante_datos_cf) FROM integrante_datos_cf WHERE idnivel_instruccion='$row[0]' ";
...
<?php
$numeroa = 1;
$sqla = " SELECT idnivel_instruccion FROM integrante_datos_cf GROUP BY idnivel_instruccion ";
...
while ($fielda = mysqli_fetch_field($resulta)){
    ...
    $sql_t = " SELECT idnivel_instruccion, nivel_instruccion FROM nivel_instruccion WHERE idnivel_instruccion='$rowa[0]' ";
    $result_t = mysqli_query($link,$sql_t);
    $row_t = mysqli_fetch_array($result_t);

    $sql_c = " SELECT count(idintegrante_datos_cf) FROM integrante_datos_cf WHERE idnivel_instruccion='$rowa[0]' ";
    $result_c = mysqli_query($link,$sql_c);
    $row_c = mysqli_fetch_array($result_c);
    $conteoa = $row_c[0];
