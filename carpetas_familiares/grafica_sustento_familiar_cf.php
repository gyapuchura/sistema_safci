...
$(function () {
    $('#corresponde_salud').highcharts({
        ...
        title: {
            text: 'INTEGRANTES QUE CONTRIBUYEN AL SUSTENTO DE LA FAMILIA - NIVEL NACIONAL'
        },
        subtitle: {
        ...
        series: [{
            ...
            data: [
                <?php
                    ...
                    $numero = 0;
                    $sql = " SELECT idcontribuye_cf FROM integrante_datos_cf GROUP BY idcontribuye_cf ";
                    ...
                    $sql_c= " SELECT count(idintegrante_datos_cf) FROM integrante_datos_cf WHERE idcontribuye_cf='$row[0]' ";
...
<?php
$numeroa = 1;
$sqla = " SELECT idcontribuye_cf FROM integrante_datos_cf GROUP BY idcontribuye_cf  ";
...
while ($fielda = mysqli_fetch_field($resulta)){
    ...
    $sql_t = " SELECT idcontribuye_cf, contribuye_cf FROM contribuye_cf WHERE idcontribuye_cf='$rowa[0]' ";
    $result_t = mysqli_query($link,$sql_t);
    $row_t = mysqli_fetch_array($result_t);

    $sql_c = " SELECT count(idintegrante_datos_cf) FROM integrante_datos_cf WHERE idcontribuye_cf='$rowa[0]' ";
    $result_c = mysqli_query($link,$sql_c);
    $row_c = mysqli_fetch_array($result_c);
    $conteoa = $row_c[0];
