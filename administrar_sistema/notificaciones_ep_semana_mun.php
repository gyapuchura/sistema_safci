...
<h4 style="font-family: Arial; font-size: 16px; color: #2D56CF; text-align: center;">Municipio : <?php echo mb_strtoupper($row1[3]);?></h4>
...
<table width="1000" border="1" align="center" cellspacing="0">
    ...
    $sql2.= " AND usuarios.idnombre=nombre.idnombre AND notificacion_ep.idmunicipio='$idmunicipio' AND registro_enfermedad.idsospecha_diag='$idsospecha_diag' GROUP BY notificacion_ep.idnotificacion_ep DESC ";
 ...
 <script>
    ...
    L.tileLayer('https://tile.openstreetmap.org/{z}/{x}/{y}.png', {
        ...
        $sql2.= " AND registro_enfermedad.idsospecha_diag='$idsospecha_diag' AND notificacion_ep.idmunicipio='$idmunicipio' GROUP BY notificacion_ep.idestablecimiento_salud  ";
        $result2 = mysqli_query($link,$sql2);
        ...
        while ($field2 = mysqli_fetch_field($result2)){
                ...
                $sql_ca.= " AND registro_enfermedad.idsospecha_diag = '$idsospecha_diag' AND notificacion_ep.idestablecimiento_salud='$row2[0]' AND notificacion_ep.idmunicipio='$idmunicipio' AND notificacion_ep.gestion='$gestion' ";
                $result_ca = mysqli_query($link,$sql_ca);    
                $row_ca = mysqli_fetch_array($result_ca);
                $casos = $row_ca[0];
            ?>
