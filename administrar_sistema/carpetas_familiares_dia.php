...
$(function () {
    $('#container').highcharts({
        ...
        title: {
            text: 'CARPETAS FAMILIARES REGISTRADOS HOY <?php echo $f_emision;?>'
        },
        legend: {
            layout: 'vertical',
            align: 'left',
 ...
 <?php
$numero = 0;
$sql = " SELECT hora_registro FROM carpeta_familiar WHERE estado='CONSOLIDADO' AND fecha_registro='$fecha' GROUP BY hora_registro ORDER BY hora_registro ";
...
<?php
$sql7 = " SELECT idcarpeta_familiar, hora_registro FROM carpeta_familiar WHERE estado='CONSOLIDADO' AND hora_registro='$row[0]' AND fecha_registro='$fecha' ";
...
<table width="1000" border="1" align="center" cellspacing="0">
    ...
    $sql.=" AND carpeta_familiar.idarea_influencia=area_influencia.idarea_influencia AND carpeta_familiar.fecha_registro='$fecha' ORDER BY carpeta_familiar.idcarpeta_familiar DESC ";
    $result = mysqli_query($link,$sql);
    if ($row = mysqli_fetch_array($result)){
    mysqli_field_seek($result,0);           
    while ($field = mysqli_fetch_field($result)){
